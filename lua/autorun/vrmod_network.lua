...
local cv_tickrate = CreateConVar("vrmod_net_tickrate", game.SinglePlayer() and "30" or "10", FCVAR_REPLICATED)
...
local function netReadFrame()
	...
	local waistPos = net.ReadVector()
	if not waistPos:IsZero() then
		frame.waistPos = waistPos
...
if CLIENT then
	...
	local cv_delay = CreateClientConVar("vrmod_net_delay", "0.3", true, false)
	local cv_delaymax = CreateClientConVar("vrmod_net_delaymax", "0.6", true, false)
	local cv_storedframes = CreateClientConVar("vrmod_net_storedframes", "10", true, false)
	...
	net.Receive("vrutil_net_pickup",function(len, ply)
		...
		for k,v in pairs(entities) do
			...
			if tmp:WithinAABox(v:OBBMins(), v:OBBMaxs()) then
				if hook.Call("VRMod_Pickup", nil, ply, v) == false then
					break
				end
