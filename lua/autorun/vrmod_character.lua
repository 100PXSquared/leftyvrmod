if CLIENT then
	...
	local function UpdateIK(ply)
		...
		if net.characterAltHead then
			local tmp1, tmp2 = WorldToLocal(Vector(0,0,0),frame.hmdAng,Vector(0,0,0),Angle(0,frame.characterYaw,0))
	...
	local function CharacterInit(ply)
		...
		if characterInfo[steamid] and characterInfo[steamid].modelName == ply:GetModel() then
			return
		end
	...
	local function PreRenderFunc()
		if g_VR.convars.oldCharacterYaw:GetBool() then
	...
	local function PrePlayerDrawFunc( ply )
		...
		if not ply:InVehicle() then
			characterInfo[steamid].renderPos = g_VR.net[steamid].lerpedFrame.hmdPos + Angle(0,g_VR.net[steamid].lerpedFrame.hmdAng.yaw,0):Forward()*-characterInfo[steamid].characterHeadToHmdDist + Angle(0,g_VR.net[steamid].lerpedFrame.characterYaw,0):Forward()*-characterInfo[steamid].horizontalCrouchOffset*0.8
